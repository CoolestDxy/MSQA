...
def buildZhRDF():
    graph = rdflib.Graph()
    for res in zhKG:
        entity1 = rdflib.URIRef(resourcePrexZh+res[0])
        relation = rdflib.URIRef(propertyPrexZh+res[1])
        entity2 = rdflib.URIRef(resourcePrexZh+res[2])
...
def buildEnRDF():
    graph = rdflib.Graph()
    for res in enKG:
        entity1 = rdflib.URIRef(resourcePrexEn+res[0])
        relation = rdflib.URIRef(propertyPrexEn+res[1])
        entity2 = rdflib.URIRef(resourcePrexEn+res[2])
        graph.add((entity1, relation, entity2))
